...
fn main() -> omron_fins::Result<()> {
    ...
    // Cancel ALL forced bits in PLC (not supported on all models; 0x04:0x41 = unsupported)
    if let Err(e) = client.forced_set_reset_cancel() {
        println!(
            "Note: forced_set_reset_cancel not supported by this PLC ({})",
            e
        );
    } else {
